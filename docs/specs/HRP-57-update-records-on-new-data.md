...
| Integration | Two concurrent enrichment attempts for the same `source_reference`/new dependent row do not both insert it; exactly one worker enriches and the other correctly reports `already_processed` | Real HRP-53 container, two threads/connections, each fully guarded so a connection failure is captured; termination and outcomes are asserted, not just absence of errors; genuine lock contention is confirmed via a bounded `pg_stat_activity` poll (an initial `Barrier` alone does not guarantee real contention, so the lock holder deterministically holds it for a bounded window) |
| Integration | Enrichment covers all four dependent tables (not only `locations`) and two distinct, concurrently-enriched employees never cross-contaminate each other's rows | Real HRP-53 container, two real database-generated `employee_id` values — a mock returning one fixed id cannot demonstrate isolation |
