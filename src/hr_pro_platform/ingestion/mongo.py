...
class MongoIngestionClient:
    ...
    def connect(self) -> None:
        ...
        logger.info(
            "Connected to MongoDB | db=%s col=%s", MONGODB_DB, MONGODB_COLLECTION
        )
    ...
    def insert_many_fragments(self, messages: list) -> bool:
        docs = []
        ...
        try:
            ...
            if code_11000:
                logger.info(
                    "Duplicate documents skipped: %d", len(code_11000)
                )
            non_duplicate = [
                e for e in bwe.details.get("writeErrors", []) if e["code"] != 11000
            ]
